...
class AutoFaissIndex:
    ...
    根据数据量自动选择最优索引类型：
    - 小数据集（<1万): FlatL2精确搜索
    - 中等数据集(1万-10万): IVFFlat近似搜索
    - 大数据集（>10万): IVFPQ高效近似搜索
    """

    def __init__(self, dimension=384): # 默认维度为384, 这是all-MiniLM-L6-v2的输出维度
...
class VectorStore:
    ...
    def search(self, query_embedding, k=10):
        ...
        if self.index is None or self.index.ntotal == 0:
            return [], [], []
        try:
            D, I = self.index.search(query_embedding, k=k) # 返回距离和索引
